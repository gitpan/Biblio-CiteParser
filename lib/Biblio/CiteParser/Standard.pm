package Biblio::CiteParser::Standard;

######################################################################
#
# Biblio::CiteParser::Standard;
#
######################################################################
#
...
use Biblio::CiteParser::Templates;
...
B<Biblio::CiteParser::Standard> - citation parsing functionality

=head1 SYNOPSIS

  use Biblio::CiteParser::Standard;
  # Parse a simple reference
  $parser = new Biblio::CiteParser::Standard;
...
Biblio::CiteParser::Standard uses a relatively simple template matching
...
=item $parser = Biblio::CiteParser::Standard-E<gt>new()
...
=item $reliability = Biblio::CiteParser::Standard::get_reliability($template)
...
=item $concreteness = Biblio::CiteParser::Standard::get_concreteness($template)
...
=item $string = Biblio::CiteParser::Standard::strip_spaces(@strings)
...
=item $templates = Biblio::CiteParser::Standard::get_templates()

Returns the current template list from the Biblio::CiteParser::Templates
...
sub get_templates
{
	return $Biblio::CiteParser::Templates::templates;
}

=pod

=item @authors = Biblio::CiteParser::Standard::handle_authors($string)
...
sub extract_metadata
{
	...
	my(@newtemplates) = ();
	foreach my $template (@$Biblio::CiteParser::Templates::templates)
	{
		$_ = $template;
		s/\\/\\\\/g;
		s/\(/\\\(/g;
	...
	{
		my $original = $Biblio::CiteParser::Templates::templates->[$index];
		if ($ref =~ /^$currtemplate$/)
		{
