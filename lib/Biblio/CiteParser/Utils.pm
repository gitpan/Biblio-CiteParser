package Biblio::CiteParser::Utils;
...
B<Biblio::CiteParser::Utils> - OpenURL utility functions
...
=item $openurl_hash = Biblio::CiteParser::Utils::trim_openurl($openurl)
...
=item $openurl_hash = Biblio::CiteParser::Utils::decompose_openurl($openurl)
...
=item $openurl = Biblio::CiteParser::create_openurl($metadata)
        ...
        foreach my $key (@openurl_keys)
        {
                if ($data_hash{$key})
                {
                        if (ref $data_hash{$key} eq "ARRAY")
                        {
                                ...
                                {
					$el =~ s/[ ]+/ /g;
                                        $openurl .= "$key=".Biblio::CiteParser::Utils::url_escape($el)."&";
                                }
                        }
                        else
                        {
				$data_hash{$key} =~ s/[ ]+/ /g;
                                $openurl .= "$key=".Biblio::CiteParser::Utils::url_escape($data_hash{$key})."&";
                        }
                }
        }
...
=item $valid_isbn = Biblio::CiteParser::Utils::isbn_check(@isbn_chars)
...
=item $sici_hash = Biblio::CiteParser::Utils::parse_sici($sici)
...
=item $bici_hash = Biblio::CiteParser::Utils::parse_bici($bici)
